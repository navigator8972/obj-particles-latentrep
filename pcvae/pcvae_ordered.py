'''
PointCloudVariationalAutoEncoder, dealing with ordered point sets so try to not use chamferloss
'''
...
if __name__ == '__main__':
    model = PointCloudVariationalAutoEncoder(n_points=1024, latent_size=16, use_pcn=-1)
    if torch.cuda.is_available():
        model.cuda()

    dataset = PointCloudDataSet('../data/Humanoid-v3-locomotions_static_ordered.npz')
    print('Training -- Number of data:{}'.format(len(dataset)))
    valid_dataset = PointCloudDataSet('../data/Humanoid-v3-locomotions_static_ordered.npz', train='valid')
    print('Validation -- Number of data:{}'.format(len(valid_dataset)))

    model.fit(dataset, valid_dataset, batch_size=32, n_epoch=3000, lr=3e-4, denoising=0.002, save_every=200, visdom=False, outf='./ordered_models')
