...
if __name__ == '__main__':
    model = PointCloudVariationalAutoEncoder(n_points=1024, latent_size=16, use_pcn=0)
    if torch.cuda.is_available():
        model.cuda()

    dataset = PointCloudDataSet('../data/Humanoid-v3-locomotions_static_reduced.npz')
    print('Training -- Number of data:{}'.format(len(dataset)))
    valid_dataset = PointCloudDataSet('../data/Humanoid-v3-locomotions_static_reduced.npz', train='valid')
    print('Validation -- Number of data:{}'.format(len(valid_dataset)))
    #model.load_model('models/pcvae_epoch_1999.pth', cuda=True)
    model.fit(dataset, valid_dataset, batch_size=4, n_epoch=4000, lr=1e-4, denoising=0.002, save_every=200, visdom=True, outf='./nonpcn_models')
